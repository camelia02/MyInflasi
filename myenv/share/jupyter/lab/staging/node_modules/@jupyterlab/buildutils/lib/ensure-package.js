...
async function ensurePackage(options) {
    ...
    let promises = Object.keys(deps).map(async (name) => {
        ...
        if (!(name in seenDeps)) {
            seenDeps[name] = await (0, get_dependency_1.getDependency)(name);
        }
        if (deps[name] !== seenDeps[name]) {
            const oneOf = deps[name].includes('||') &&
                deps[name]
                    .split(/\|\|/)
                    .map(v => v.trim())
    ...
    if (data.name.indexOf('example') === -1 &&
        ...
        imports.forEach(importStr => {
            if (importStr.indexOf('.css') !== -1 &&
                ...
                messages.push('CSS imports are not allowed source files unless using `.raw.css` extension');
            }
        });
    }
    ...
    if (usesTS) {
        if (cssImports.length > 0 ||
            ...
            messages.push(...(await ensureFile(cssIndexPath, cssIndexContents.join('\n'), false)));
            ...
            messages.push(...(await ensureFile(jsIndexPath, jsIndexContents.join('\n'), false)));
        }
        else {
    ...
    const ignoreDirs = ['.ipynb_checkpoints'];
    // Function to get all of the files in a directory, recursively.
    function recurseDir(dirname, files, skipDirs = ignoreDirs) {
    ...
    if (path.basename(pkgPath) != 'galata' &&
        (buildScript === null || buildScript === void 0 ? void 0 : buildScript.indexOf('tsc')) === -1 &&
        !isPrivate) {
 ...
 */
async function ensureFile(fpath, contents, prettify = true) {
    const messages = [];
